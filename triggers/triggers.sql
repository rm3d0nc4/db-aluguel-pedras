...
CREATE OR REPLACE FUNCTION T_ATUALIZAR_VALOR_TOTAL() 
RETURNS TRIGGER AS $$
	DECLARE
	VAR_NOVO_VALOR_TOTAL NUMERIC(10,2);
	
	BEGIN
		SELECT COALESCE(SUM(VALOR_DIARIA * (DATA_FINAL - DATA_INICIAL)), 0) INTO VAR_NOVO_VALOR_TOTAL FROM LISTA_PEDRAS_PEDIDO NATURAL JOIN PEDRA WHERE ID_PEDIDO = COALESCE(NEW.ID_PEDIDO, OLD.ID_PEDIDO);

		UPDATE PEDIDO SET VALOR_TOTAL = VAR_NOVO_VALOR_TOTAL WHERE ID_PEDIDO = COALESCE(NEW.ID_PEDIDO, OLD.ID_PEDIDO);
		RETURN COALESCE(NEW, OLD);
	END;

$$ LANGUAGE 'plpgsql';

CREATE OR REPLACE TRIGGER TRIGGER_ATUALIZAR_VALOR_TOTAL AFTER INSERT OR UPDATE OR DELETE ON LISTA_PEDRAS_PEDIDO FOR EACH ROW WHEN (NEW) EXECUTE PROCEDURE T_ATUALIZAR_VALOR_TOTAL();

-- DROP TRIGGER TRIGGER_ATUALIZAR_VALOR_TOTAL ON LISTA_PEDRAS_PEDIDO; -- Bloqueia a função FATURAR_PEDRA
